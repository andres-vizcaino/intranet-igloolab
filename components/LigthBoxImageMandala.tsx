...
import { likeMandala, unlikeMandala } from 'services/mandala.services'
...
const LigthBoxImageMandala = ({
  ...
  const handleClickLike = async () => {
    ...
    if (!isLiked) {
      setLikes(likes + 1)
      await likeMandala(idPet, session?.user?.id)
    } else {
      setLikes(likes - 1)
      await unlikeMandala(idPet, session?.user?.id)
    }
  }
...
export default LigthBoxImageMandala
